...
def run():
    ...
    res = run_lcu_simulation(q=q, mass=mass, omega=omega, max_x=max_x, t=t, K=K, threshold=threshold, time_steps=time_steps)
    ...
    # Generate the circuit plot
    print("\nGenerating and saving circuit diagram using matplotlib...")
    qc = res['circuit']
    
    # Using 'mpl' output utilizes matplotlib and pylatexenc to draw a beautiful schematic
    fig = qc.draw(output='mpl', style='clifford')
    
    # Save the circuit to a file
    filepath = 'lcu_circuit_diagram.png'
    fig.savefig(filepath, dpi=300, bbox_inches='tight')
    print(f"Saved circuit to '{filepath}'.")
